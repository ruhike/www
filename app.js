...
function getLengthClass(length) {
    const km = parseFloat(length.replace('公里', ''));
...
function getTimeClass(time) {
    const hours = parseFloat(time.replace('小时', ''));
...
function getDifficultyStars(ascent) {
    const solidStar = '★';
    // 每500米爬升对应1颗星，最高9颗星
    const level = Math.min(Math.floor(ascent / 500) + 1, 9);
    return solidStar.repeat(level) + '☆'.repeat(9 - level);
}
...
// 渲染省份列表
// 根据区域获取随机线路
function getRandomHikingsByArea(province, city, county, count = 12) {
    let hikings = [];
